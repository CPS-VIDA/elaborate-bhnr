...
class PPO2(ActorCriticRLModel):
    ...
    def set_monitor(self, monitor: STLMonitor):
        self.monitor = monitor

    def learn(self, total_timesteps, callback=None, seed=None, log_interval=1, tb_log_name="TLPPO2",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            runner = Runner(env=self.env, monitor=self.monitor, model=self, n_steps=self.n_steps, gamma=self.gamma,
                            lam=self.lam)
...
class Runner(AbstractEnvRunner):
    def __init__(self, *, env, model, monitor: STLMonitor, n_steps, gamma, lam):
        """
    ...
    def run(self):
        ...
        mb_rewards = self.monitor.get(mb_obs, mb_dones, num_envs)
